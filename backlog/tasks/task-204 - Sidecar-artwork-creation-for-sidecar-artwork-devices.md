...
Implement sidecar artwork file creation for devices that read artwork from files alongside the audio (e.g., `folder.jpg`). The sidecar is always created at the device's optimal resolution, while transfer mode controls what happens to embedded artwork.
