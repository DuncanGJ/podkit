/**
 * Unified sync planner that delegates to ContentTypeHandler
 *
...
import type { UnifiedSyncDiff } from './unified-differ.js';
...
/**
 * Options for unified sync planning
 */
export interface UnifiedPlanOptions extends HandlerPlanOptions {
...
/**
 * Result of planning a single add operation, including any warnings
 */
...
export class UnifiedPlanner<TSource, TDevice> {
  ...
  plan(diff: UnifiedSyncDiff<TSource, TDevice>, options?: UnifiedPlanOptions): SyncPlan {
    ...
    for (const update of diff.toUpdate) {
      ...
      const ops = this.handler.planUpdate(update.source, update.device, reasons);
...
/**
 * Create a UnifiedPlanner for the given content type handler
 *
 * @param handler - ContentTypeHandler implementation
 * @returns A new UnifiedPlanner instance
 */
export function createUnifiedPlanner<TSource, TDevice>(
  handler: ContentTypeHandler<TSource, TDevice>
): UnifiedPlanner<TSource, TDevice> {
