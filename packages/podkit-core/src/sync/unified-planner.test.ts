/**
 * Unit tests for the unified planner
 *
 * Tests the generic UnifiedPlanner that delegates to ContentTypeHandler.
 * Uses a minimal mock handler to verify the algorithm independent of
 * any specific content type.
 */

import { describe, expect, it } from 'bun:test';
import { UnifiedPlanner, createUnifiedPlanner, orderOperations } from './unified-planner.js';
...
describe('UnifiedPlanner', () => {
  ...
  describe('createUnifiedPlanner factory', () => {
    it('should create a working planner instance', () => {
      const handler = createMockHandler();
      const planner = createUnifiedPlanner(handler);

      expect(planner).toBeInstanceOf(UnifiedPlanner);
