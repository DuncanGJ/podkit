/**
 * UnifiedExecutor — generic sync executor that delegates to ContentTypeHandler
 *
...
/**
 * Options for unified sync execution
 */
export interface UnifiedExecuteOptions {
    ...
    transcodeProgress: progress.transcodeProgress
      ? ({
          ...
          speed: progress.transcodeProgress.speed
            ? parseFloat(progress.transcodeProgress.speed) || undefined
            : undefined,
 ...
 */
export class UnifiedExecutor<TSource, TDevice> {
    ...
    for (let index = 0; index < plan.operations.length; index++) {
      ...
      try {
        ...
        for await (const progress of gen) {
          yield buildExecutorProgress(progress, index, total, displayName, {
            bytesTotal: plan.estimatedSize,
          });
        }
        completed++;
    ...
    try {
      ...
      for await (const progress of gen) {
        ...
        if (progress.phase === 'failed') {
          ...
          yield buildExecutorProgress(progress, operationIndex, total, displayName, {
            bytesTotal: plan.estimatedSize,
            categorizedError: catError,
          });
...
/**
 * Create a UnifiedExecutor for a given content type handler
 */
export function createUnifiedExecutor<TSource, TDevice>(
  handler: ContentTypeHandler<TSource, TDevice>
): UnifiedExecutor<TSource, TDevice> {
